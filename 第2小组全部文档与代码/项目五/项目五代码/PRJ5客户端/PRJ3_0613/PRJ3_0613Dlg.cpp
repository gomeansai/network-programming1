...
void CPRJ3_0613Dlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, LiaoTianJiLuXianShi, m_LiaoTianJiLuXianShi);
	DDX_Control(pDX, HaoYouLieBiao, m_HaoYouLieBiao);
}
...
BEGIN_MESSAGE_MAP(CPRJ3_0613Dlg, CDialogEx)
	...
	ON_WM_TIMER()
	ON_BN_CLICKED(FaSongXiaoXiAnNiu, &CPRJ3_0613Dlg::OnBnClickedFasongxiaoxianniu)
END_MESSAGE_MAP()
...
void CPRJ3_0613Dlg::OnBnClickedShangxiananniu()
{
	...
	CString addr_str, addr_forbin, addr_empty;
	...
	LocalAddr.sin_port = htons(LOCAL_CTL_PORT);
	GetDlgItemText(BenDiIPShuRuKuang, addr_str);
	LocalAddr.sin_addr.S_un.S_addr = inet_addr(addr_str.GetBuffer(sizeof(addr_str)));
	addr_forbin = "127.0.0.1";
	addr_empty = "";
	if (addr_str == addr_empty)
	{
		MessageBox((LPCTSTR)"左下角本地IP地址未填写（回环测试下需填127.0.0.X,X=2~254）", (LPCTSTR)"本地", MB_OK);
		return;
	}
	if (addr_str == addr_forbin)
	{
		MessageBox((LPCTSTR)"回环测试下，不能绑定127.0.0.1", (LPCTSTR)"本地", MB_OK);
		return;
	}

	if (bind(CtlSock, (sockaddr*)&LocalAddr, sizeof(LocalAddr)) != 0){
		iResult = WSAGetLastError();
		MessageBox((LPCTSTR)"显式绑定失败", (LPCTSTR)"本地", MB_OK);
		return;
	}
	...
	MyAddr.sin_addr.S_un.S_addr = LocalAddr.sin_addr.S_un.S_addr;
...
LRESULT CPRJ3_0613Dlg::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)		//消息处理程序（自定义）
{
	...
	case WM_SOCKET:
		...
		{
		case FD_READ:
			char tempcmdbuff[BUF_SIZE];
			len = recv(CtlSock, tempcmdbuff, BUF_SIZE, 0);
			if (len <= 0){
			...
			case CTL_PDU_TYPE:
				...
				case REQ_TRANSLATE:
					CtlPDU *ctl_pdu;
					ctl_pdu = (CtlPDU*)tempcmdbuff;
					CString str_id;
					str_id.Format("%d", ctl_pdu->id);
					CString info;
					info = "用户" + str_id + "向你发送文件：" + ctl_pdu->buf;
					retval = MessageBox(info, _T("文件直传"), MB_YESNO);
					if (retval == IDYES){
						//如何确认接收
						int i;
						...
						sock_num += 1;
						sock_list[i] = -1;
						FileName[i] = ctl_pdu->buf;
						FilePath[i] = "C:\\ClientData\\";
						FilePath[i] += ctl_pdu->buf;

						strcat(ctl_pdu->buf, "YES");
						ctl_pdu->len += 3;
					}
					else if(retval == IDNO){
						strcat(ctl_pdu->buf, "NO");
						ctl_pdu->len += 2;
					}

					int buf_len = ctl_pdu->len + 4;
					retval = send(s, (char*)ctl_pdu, buf_len, 0);
					break;
				}
				break;
				//6.17：增加对聊天类型数据包的处理
			case CHAT_PDU_TYPE:
			{
				...
				case ADD_FRD:
					...
					for (int i = 0; i < LISTMAXLEN; i++)
					{
						if (infomanage[i].id < 0){
							infomanage[i].id = chat_buff->frdno;
							strcpy(infomanage[i].name, chat_buff->msg);
							break;
						}
					}
					Modify_Friend_List();
				...
				case DEL_FRD:
					...
					for (int i = 0; i < LISTMAXLEN; i++)
					{
						if (infomanage[i].id == chat_buff->frdno)
						{
							infomanage[i].id = -1;
							infomanage[i].num = -1;
							break;
				...
				case FRD_MSG:
					for (int i = 0; i < LISTMAXLEN; i++)
					{
						if (infomanage[i].id == chat_buff->frdno){
							//新插入的数据在下面
							int nIndex = m_LiaoTianJiLuXianShi.GetItemCount();
							LV_ITEM lvItem;
							...
							//在最后一行插入记录值.
							m_LiaoTianJiLuXianShi.InsertItem(&lvItem);
							//插入其它列
							CString  strTime;
							CTime time;
							time = CTime::GetCurrentTime();
							strTime = time.Format("%H:%M:%S");
							m_LiaoTianJiLuXianShi.SetItemText(nIndex, 1, strTime);
							m_LiaoTianJiLuXianShi.SetItemText(nIndex, 2, chat_buff->msg);
							break;
						}
					}
					break;
				case BROCAST_MSG:
					MessageBox(TEXT("客户机不应接收广播消息！"), TEXT("接收异常"), MB_OK);
					break;
				}
			}
			break;
...
{
	m_ShangChuanLuJing.ResetContent();
	// TODO:  在此添加控件通知处理程序代码
...
void CPRJ3_0613Dlg::OnBnClickedShangchuananniu()
{
	...
	int buf_len;
	unsigned char id;
	int nIndex = m_HaoYouLieBiao.GetCurSel();
	...
	if (nIndex > 0)
	{
		...
		MessageBox(_T("请选择正确的好友"), _T("上传错误"), MB_OK);
		return;
	}

	CtlPDU *ctl_pdu = (CtlPDU *)malloc(sizeof(CtlPDU));
	ctl_pdu->type = CTL_PDU_TYPE;
	ctl_pdu->cmd = REQ_UPFILE;
	ctl_pdu->id = id;
	CString strFileName;
	//m_ShangChuanLuJing.GetText(0, pathstr);
	strFileName = strPath.Mid(strPath.ReverseFind('\\') + 1);
	//strFileName += '\0';
	ctl_pdu->len = strFileName.GetLength();
	strcpy(ctl_pdu->buf, strFileName.GetBuffer(sizeof(strFileName)));

	buf_len = ctl_pdu->len + 4;
	send(CtlSock, (char *)ctl_pdu, buf_len, 0);
...
void CPRJ3_0613Dlg::OnBnClickedXiazaianniu()
{
	...
	CtlPDU *ctl_pdu = (CtlPDU *)malloc(sizeof(CtlPDU));
	ctl_pdu->type = CTL_PDU_TYPE;
	ctl_pdu->cmd = REQ_DOWNFILE;
	ctl_pdu->len = strText.GetLength();
	ctl_pdu->id = 0;
	strcpy(ctl_pdu->buf, strText.GetBuffer(sizeof(strText)));
...
void CPRJ3_0613Dlg::Re_Account(SOCKET sock)
{
	...
	name += '\0';
	ctl_pdu->type = CTL_PDU_TYPE;
	ctl_pdu->cmd = REQ_ACCOUNT;
	ctl_pdu->len = name.GetLength();
	strcpy(ctl_pdu->buf, name.GetBuffer(sizeof(name)));

	buf_len = 1 + 1 + 1 + ctl_pdu->len;
	retval = send(CtlSock, (char *)ctl_pdu, buf_len, 0);
	if (retval < buf_len){
...
void CPRJ3_0613Dlg::AcceptDataSocket(SOCKET s)
{
	...
	CString info = file_pdu->buf;
	MessageBox(info, _T("文件名"), MB_OK);
	for (i = 0; i < SOCK_LIST_SIZE; i++){
...
void CPRJ3_0613Dlg::ChatGUIInit()
{
	LONG lStyle;
	lStyle = GetWindowLong(m_LiaoTianJiLuXianShi.m_hWnd, GWL_STYLE);//获取当前窗口style
	lStyle &= ~LVS_TYPEMASK; //清除显示方式位
	lStyle |= LVS_REPORT; //设置style
	SetWindowLong(m_LiaoTianJiLuXianShi.m_hWnd, GWL_STYLE, lStyle);//设置style
	DWORD dwStyle = m_LiaoTianJiLuXianShi.GetExtendedStyle();
	dwStyle |= LVS_EX_FULLROWSELECT;//选中某行使整行高亮（只适用与report风格的listctrl）
	dwStyle |= LVS_EX_GRIDLINES;//网格线（只适用与report风格的listctrl）
	m_LiaoTianJiLuXianShi.SetExtendedStyle(dwStyle); //设置扩展风格
	m_LiaoTianJiLuXianShi.InsertColumn(0, (LPCTSTR)_T("昵称"), LVCFMT_CENTER, 50);	//插入相应的列，下同
	m_LiaoTianJiLuXianShi.InsertColumn(1, (LPCTSTR)_T("时间"), LVCFMT_CENTER, 50);
	m_LiaoTianJiLuXianShi.InsertColumn(2, (LPCTSTR)_T("消息"), LVCFMT_CENTER, 150);
}

void CPRJ3_0613Dlg::OnBnClickedFasongxiaoxianniu()
{
	// TODO:  在此添加控件通知处理程序代码
	int i;
	CString cs_msg_in;
	ChatPDU *ChatBuff = (ChatPDU *)malloc(sizeof(ChatPDU));
	int nIndex = m_HaoYouLieBiao.GetCurSel();
	ChatBuff->type = CHAT_PDU_TYPE;
	if (nIndex == 0)
	{
		ChatBuff->cmd = BROCAST_MSG;
	}
	else
	{
		for (i = 0; i < LISTMAXLEN; i++)
		{
			if (infomanage[i].num == nIndex)
			{
				ChatBuff->frdno = infomanage[i].id;
				ChatBuff->cmd = FRD_MSG;
				break;
			}
		}
	}
	GetDlgItemText(XiaoXiShuRuKuang, cs_msg_in);
	cs_msg_in += '\0';
	strcpy(ChatBuff->msg, cs_msg_in.GetBuffer(sizeof(cs_msg_in)));
	unsigned int msglen = strlen(ChatBuff->msg)+1;
	unsigned int sendlen = msglen + 4;
	send(CtlSock, (char *)ChatBuff, sendlen, 0);
	GetDlgItem(XiaoXiShuRuKuang)->SetWindowText(_T(""));


	CString LocalName;
	GetDlgItemText(ZhangHaoShuRuKuang, LocalName);
	LocalName += '\0';
	nIndex = m_LiaoTianJiLuXianShi.GetItemCount();
	LV_ITEM lvItem;
	lvItem.mask = LVIF_TEXT;
	lvItem.iItem = nIndex; //行数
	lvItem.iSubItem = 0;
	lvItem.pszText = LocalName.GetBuffer(sizeof(LocalName)); //第一列
	//在最后一行插入记录值.
	m_LiaoTianJiLuXianShi.InsertItem(&lvItem);
	//插入其它列
	CString  strTime;
	...
	strTime = time.Format("%H:%M:%S");
	m_LiaoTianJiLuXianShi.SetItemText(nIndex, 1, strTime);
	m_LiaoTianJiLuXianShi.SetItemText(nIndex, 2, cs_msg_in);
}

void CPRJ3_0613Dlg::Modify_Friend_List()		//好友列表维护程序，包括重新排序与显示
{
	int serial = 1;
	for (int i = 0; i < LISTMAXLEN; i++)
	{
		if (infomanage[i].id >0 || infomanage[i].id == 0)
			infomanage[i].num = serial++;
	}
	m_HaoYouLieBiao.ResetContent();
	m_HaoYouLieBiao.InsertString(0, "群发");
	for (int i = 1; i < FrdListLen + 1; i++)
	{
		for (int j = 0; j < LISTMAXLEN; j++)
		{
			if (infomanage[j].num == i)
			{
				m_HaoYouLieBiao.InsertString(i, infomanage[j].name);
				break;
			}
		}
	}
