...
#define UM_TRANS_FILE_SOCK UM_SOCK + 4

//接收缓存区大小---> 比 FIlePDU 大 1
#define BUF_SIZE	FILE_BUF_SIZE + 1 + 1 + 1 + 1 + 1
...
#define CHAT_PDU_TYPE 6
#define CHAT_BUF_SIZE 255     //一个字节所能存储的最大数值
#define ADD_FRD 60
#define DEL_FRD 61
#define FRD_MSG 62
#define BROCAST_MSG 63
typedef struct ChatPDUS{
	unsigned char type;      //CTL_PDU_TYPE = 2
	unsigned char cmd;		//命令，包括好友上线、下线、单播信息、广播信息
	unsigned char frdno;	//好友ID
	unsigned char len;
	char msg[CHAT_BUF_SIZE];
}ChatPDU;
...
#define REQ_TRANSLATE	24	//Server to Client, Translate File
